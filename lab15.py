...
def direct_tree(cell_now: Cell):
    if cell_now is None:
        return
    print(cell_now.value, end=' ')
    direct_tree(cell_now.left)
    direct_tree(cell_now.right)


def central_tree(cell_now: Cell):
    if cell_now is None:
        return
    direct_tree(cell_now.left)
    print(cell_now.value, end=' ')
...
def terminal_tree(cell_now: Cell):
    if cell_now is None:
        return
    direct_tree(cell_now.left)
    direct_tree(cell_now.right)
...
if __name__ == '__main__':  # 8 (3 (1, 6 (4,7)), 10 (, 14(13,)))
    ...
    print("Прямой обход:")
    direct_tree(cell_tree)
    print()
    print("Центральный обход:")
    central_tree(cell_tree)
    print()
    print("Концевой обход:")
    terminal_tree(cell_tree)
    print()
